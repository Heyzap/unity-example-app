...
namespace UnityEditor.XCodeEditor.FyberPlugin
{
	public partial class XCProject : System.IDisposable
	{
		...
		public bool AddLibrarySearchPaths( PBXList paths)
		{
			...
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				buildConfig.Value.AddLibrarySearchPaths( relativePaths, false );
			}
			modified = true;
			return modified;
		}
		...
		public bool AddHeaderSearchPaths( PBXList paths )
		{
			...
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				buildConfig.Value.AddHeaderSearchPaths( relativePaths, false );
			}
			modified = true;
			return modified;
		}
		...
		internal static string RemoveXcodeQuotes(string path)
		{
