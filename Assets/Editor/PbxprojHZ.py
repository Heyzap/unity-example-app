...
class PbxprojHZ(object):

	@staticmethod
	def get_pbxproj_by_name(name, xcode_version = None):
		if name not in pbxproj_cache:
			pbxproj_cache[name] = PbxprojHZ(name, xcode_version = xcode_version)
	...
	def path(self):
		...
		if re.match('^[a-zA-Z0-9\.\-:+"]+$', self.name):
			return os.path.join(PathsHZ.src_dir, self.name.strip('"'), self.name.strip('"')+'.xcodeproj', 'project.pbxproj')
	...
	def add_buildfile(self, name, file_ref_hash, default_guid, weak_link=False):
		project_data = self.get_project_data()
		...
		if buildfile_hash is None:
			...
			if weak_link:
				libfiletext = libfiletext + " settings = {ATTRIBUTES = (Weak, ); };"
			libfiletext = libfiletext + " };\n"
	...
	def get_rel_path_to_products_dir(self):
		...
		build_path = os.path.join(os.path.join(os.path.dirname(PathsHZ.src_dir), 'Build'), 'Products')
	...
	def add_header_search_path(self, configuration):
		project_path = os.path.dirname(os.path.abspath(self.xcodeprojpath()))
		build_path = os.path.join(os.path.join(os.path.join(os.path.dirname(PathsHZ.src_dir), 'Build'), 'Products'), 'three20')
	...
	def add_bundle(self, path):
		tthash_base = self.get_hash_base(path)
	 	fileref_hash = self.add_filereference(path, 'plug-in', tthash_base+'0', path, 'SOURCE_ROOT')
	 	libfile_hash = self.add_buildfile(path, fileref_hash, tthash_base+'1')

		if not self.add_file_to_resources(path, fileref_hash):
			return False

		if not self.add_file_to_resources_phase(path, libfile_hash):
			return False
